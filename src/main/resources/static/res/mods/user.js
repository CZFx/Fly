...
layui.define(['laypage', 'fly', 'element', 'flow'], function(exports){
  ...
  $('#userLogin').submit(function () {
    ...
    fly.json('/user/login', {
      ...
      //成功返回
      if (res.data === 0){  //登陆成功，跳转到主页
        layer.msg('登陆成功，即将跳转至主页');
        window.location.href = '/';
      } else if (res.data === 1) {  //验证码错误，清空验证码input并更换验证码
        layer.msg(res.msg);
        vercode.val('');
        $('.vercode').attr("src", "/vercode?"+new Date().getTime());
      } else if (res.data === 2) {  //密码错误，清空密码input
        layer.msg(res.msg);
        password.val('');
        $('.vercode').attr("src", "/vercode?"+new Date().getTime());
      } else {   //其他错误
